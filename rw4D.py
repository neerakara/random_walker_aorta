...
def _make_graph_edges_4d(n_x, n_y, n_z, n_t):
    ...
    edges_time = np.vstack((vertices[:,:,:,:-1].ravel(), vertices[:,:,:,1:].ravel()))
...
def _compute_dotproduct_4d(data, spacing, c):
    ...
    dot_time_magn = ((utils.norm(data[:, :, :, :-1, 1], data[:, :, :, :-1, 2], data[:, :, :, :-1, 3])).ravel() * (utils.norm(data[:, :, :, 1:, 1], data[:, :, :, 1:, 2], data[:, :, :, 1:, 3])).ravel())
...
def _make_laplacian_sparse(edges, weights):
    ...
    lap = sparse.coo_matrix((np.hstack((data, connect)), (np.hstack((i_indices, diag)),
...
def _mask_edges_weights(edges, weights, mask):
    ...
    order = np.searchsorted(np.unique(edges.ravel()), np.arange(max_node_index + 1))
    edges = order[edges.astype(np.int64)]
    return edges, weights

def _build_laplacian(data,
                     spacing,
                     mask=None,
                     alpha = 0.3,
                     beta = 0.3,
    ...
    weights = _compute_weights_4d(data,
                                  spacing,
                                  eps=1.e-10,
                                  alpha=alpha,
                                  beta=beta,
